...
df['daily_cash_risk_target'] = daily_cash_risk
...
df[['ideal_pos_contracts']].to_csv('ideal_pos_contracts.csv')
...
strat_std_dev = strat_pct_returns.ewm(span=35, min_periods=10).std()
strat_sr = np.sqrt(trading_days_in_year) * (strat_pct_returns.mean() / strat_std_dev)

strat_raw_usd_std_dev_pre = strat_raw_usd_returns.ewm(span=35, min_periods=10).std()
strat_raw_usd_std_dev_post = strat_usd_returns.ewm(span=35, min_periods=10).std()
...
def plot(column_name1, column_name2):
    ...
    instr_vol = df['instr_perc_returns'].ewm(span=35, min_periods=10).std()
